...
from telco_churn.cli import build_parser
from telco_churn.paths import prepare_output_dir


def test_cli_accepts_arbitrary_input_path_for_validation():
    args = build_parser().parse_args(
        ["validate-data", "--input", "/tmp/custom-workbook.xlsx"]
    )
    assert args.command == "validate-data"
    assert args.input == Path("/tmp/custom-workbook.xlsx")
